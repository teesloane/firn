...
as well as how to render a list of tags on a Firn site.
...
If you /do/ choose to change the name of your tag file, make sure you alter the ~tag-file~ value in your ~config.edn~ - this ensures that every tag that displays next to an org-mode heading links to the proper url.
